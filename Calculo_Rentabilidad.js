...
document.getElementById('calculatorForm').addEventListener('submit', function(event) {
  ...
  data.push({calculoscount: calculoscount,  Gestion: Gestion, Mes: Mes, articleName: articleName, utilities: utilities, salesIncome: salesIncome, totalSalesIncome: totalSalesIncome, variableCost: variableCost});
  ...
  row.innerHTML = '<td>' + calculoscount
  + '</td><td>' + articleName  
...
function addData(){
  ...
  dataTable.innerHTML = '<tr><th>ID</th><th>Artículo</th><th>Utilidades</th><th>Ingreso por Venta</th><th>Ingresos Totales por Venta</th><th>Costo Variable</th></tr>';
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      dataRow.insertCell(0).innerHTML = data[i].calculoscount;
